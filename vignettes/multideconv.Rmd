...
res = pipeML::compute_features.training.ML(deconv_train, traitData_train$Response, trait.positive = "R", 
                                   metric = "AUROC", stack = F, k_folds = 5, n_rep = 10, feature.selection = F, 
                                   seed = 123, LODO = F, ncores = 3, return = F, fold_construction_fun = prepare_multideconv_folds)

# Predict in deconv subgroups
dt_test = replicate_deconvolution_subgroups(res$Custom_output$Processed_deconvolution, deconv_test) #Replicate deconvolution subgroups
pred = pipeML::compute_prediction(res, dt_test, traitData_test$Response, trait.positive = "R", 
                                  stack = F, maximize = "Accuracy", return = T)
```
