...
When your samples come from multiple cohorts or batches, simple Pearson/Spearman correlations can be confounded by cohort structure. `compute.deconvolution.analysis()` accepts a `batch` argument that switches the internal correlation to **partial correlation**, controlling for cohort membership at every grouping step (pairwise pruning and WGCNA subgrouping).

The `batch` vector must be a named factor or character vector whose names match the row names of the deconvolution matrix.
...
- Pairwise correlation pruning uses **partial correlation** (residuals after regressing out batch) via `ppcor::pcor()`.
- WGCNA subgrouping uses the same partial-correlation matrix so that inter-cohort differences do not inflate feature similarity.
...
### Option A — Supply a pre-computed pathway matrix

Any sample × pathway numeric matrix can be used. A popular choice is **PROGENy**, which models the activity of 14 cancer-relevant signalling pathways from gene expression:

```{r, eval = FALSE}
# Install PROGENy if needed: BiocManager::install("progeny")
library(progeny)

counts     <- multideconv::raw_counts
counts_tpm <- ADImpute::NormalizeTPM(counts, log = FALSE)

# PROGENy returns a sample × pathway matrix
pathway_scores <- progeny(counts_tpm, scale = FALSE, organism = "Human",
...
### Option B — Compute PROGENy internally

If you pass `counts_norm` instead of `pathways`, the function will compute PROGENy activity scores internally (requires the `progeny` Bioconductor package):
...
One PDF heatmap per cell type is saved to `Results/`. Each heatmap shows Pearson correlation between deconvolution subgroups (rows) and pathway activity scores (columns). Stars indicate significance (* p<0.05, ** p<0.01, *** p<0.001).
